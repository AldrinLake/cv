  ...
  function render(data, lang) {
    ...
    sidebar.innerHTML = `
      <section class="profile-card">
        ...
        <p class="contact-line"><a class="contact-link" href="mailto:${escapeHtml(left.email)}" aria-label="${escapeHtml(labels.email)}: ${escapeHtml(left.email)}">${getContactIcon('email')}<span>${escapeHtml(labels.email)}: ${escapeHtml(left.email)}</span></a></p>
